         ...
         `(,@func-name (&rest args)
                       ...
                       (if (and (listp (car args))
                           ...
                           (let* ((kwargs (cdar args))
                             ...
                             (setq pos+optional (list-to-vector pos+optional))
                             ;;make alist of index value pairs
                             (setq index-value
                                   (mapcar (lambda (kw)
                                             (if (setq arg-index
                                                       (assoc (car kw) ',arg-index-alist))
                                                 (progn (if (< (cdr arg-index) len)
                                                            (signal 'TypeError (format ,(format "%s() got multiple values for keyword argument '%%s'" name) (caddr iv))))
                                                        (aset pos+optional (cdr arg-index) (cdr kw)) 
                                                        (setq kwargs-used (1+ kwargs-used))
                                                        (setq kwargs
                                                              (remove (assoc (car arg-index)) kwargs)) 
                                                        )))
                                           kwargs))

                             (setq pos+optional (mapcar 'identity pos+optional))
