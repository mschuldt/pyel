                       ...
                       (if (and (listp (car __pyel_args))
                           ...
                           (let* ((__pyel_kwargs (cdar __pyel_args))
                             ...
                             (cond ((= __pyel_len ,nargs)
                                   ...
                                   ((> __pyel_len ,nargs)
                                    (setq __pyel_pos+optional (subseq __pyel_args 0 ,nargs)
                                          ,rest (subseq __pyel_args ,nargs)))
                                   (t ;;(< __pyel_len ,nargs)
                                    (setq __pyel_pos+optional
                                          (append __pyel_args (make-list (- ,nargs __pyel_len) nil)))))
                             ...
                             (if (< (+ __pyel_kwargs-used __pyel_len) ,npositional)
                                 (signal 'TypeError (format ,(format "%s() takes at least %s arguments (%%s given)" name npositional) (+ __pyel_kwargs-used __pyel_len))))
                             
                             ,@(mapcar (lambda (x)
                                         `(setq ,x (car __pyel_pos+optional)
                                                __pyel_pos+optional (cdr __pyel_pos+optional)))
                                       (append positional optional))
                             ...
                             ;;          ,@body)
                             ;;        (append __pyel_pos+optional
                             ;;                ,(if kwarg
                             ;;                     `(list rest (pyel-alist-to-hash __pyel_kwargs))
                             ;;                   `(list rest))))
